...
# Load Dataset
st.header("Dataset")
uploaded_file = st.file_uploader(r"C:\Users\benne\OneDrive\CIS 9660- Data Mining - Summer 2025\Project 2\hypertension-risk-prediction-dataset.zip")

hypertension_df = None
if uploaded_file is not None:
    try:
        hypertension_df = pd.read_csv(uploaded_file)
...
if hypertension_df is not None:
    ...
    # Display data
    st.subheader("Data Preview Rows")
    st.dataframe(hypertension_df.head())

    # Dataset description
    st.subheader("Dataset Description (Statistics)")
    ...
    # Verify change
    st.subheader("Medication Value Counts")
    ...
    input_gender = st.selectbox("Gender", hypertension_df['Gender'].unique())
    input_smoking = st.selectbox("Smoking", hypertension_df['Smoking'].unique())
    input_alcohol_consumption = st.selectbox("Alcohol Consumption", hypertension_df['Alcohol_Consumption'].unique())
    input_diet = st.selectbox("Diet", hypertension_df['Diet'].unique())
    input_physical_activity = st.selectbox("Physical Activity", hypertension_df['Physical_Activity'].unique())
    ...
    input_data = pd.DataFrame({
        ...
        'Smoking': [input_smoking],
        'Alcohol_Consumption': [input_alcohol_consumption],
        'Diet': [input_diet],
        'Physical_Activity': [input_physical_activity]
    })

    # Apply one-hot encoding to user input, ensuring all columns are present
    # Reindex input_data to match the columns of X_encoded (training data)
